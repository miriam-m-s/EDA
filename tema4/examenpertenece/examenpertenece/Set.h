...
public:
    ...
    bool operator<=(const Set<T>& other) const{ //coste n

       // int i = 0;
        //while (i < nelems && set.contains(array[i])) { i++; }
        ...
        while (i < nelems && hey) {
            ...
            bool g = true;
            while (j < other.nelems && g)
            { 
            if (array[i] == other.array[j])g = false;
            j++;
            }
            if (j == other.nelems&&g )hey= false;
