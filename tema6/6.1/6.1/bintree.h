...
public:
    ...
    int nodos() {
     ...
     void  cuentahojas(Link a, int& s) {
         if (a != nullptr) {
             if (a->left == nullptr&&a->right==nullptr)s++;
             else {
                 cuentahojas(a->left, s);
                 cuentahojas(a->right, s);
             }                
         }
     }
     int camino() {
