...
public:
    ...
    void fam(bool& hey, int& levels) {
        
        if (!empty()) {
            hey = familia(raiz, levels, 1);
        }
       
    }
    bool familia(Link a,int& levels, int k ) {
        if (a != nullptr) {
            if (a->left != nullptr) {
                if (a->left->elem <= a->elem - 18) {
                    if (a->right != nullptr) {
                        if (a->left->elem - 2 >= a->right->elem) {
                            return (familia(a->left, levels, k + 1) && familia(a->right, levels, k + 1));
                        }
                        else return false;
                    }
                    else {
                        return familia(a->left, levels, k + 1);
                    }
                }
                else return false;
            }
