...
public:
    int camino() {
        ...
        int s = 0;
        int l = 0;
        caminouno(raiz, maxcam, s, l);
        return maxcam;
    }
    void caminouno(Link a, int& maxcam, int& maxizq, int& maxder) {
        if (a != nullptr) {
            bool h = true;
            bool fin = false;
            int m = max(a, h, fin, maxizq, maxder);
            if (m > maxcam)maxcam = m;
            if (!fin) {
                if (h) {
                    maxder = 0;
                    caminouno(a->left, maxcam, maxizq, maxder);
                }

                else {
                    maxizq = 0;
                    caminouno(a->right, maxcam, maxizq, maxder);
                }
    ...
    int max(Link a, bool& hey, bool& fin, int& maxizq, int& maxder) {
        if (maxizq == 0)
            maxizq = maxcamino(a->left);
        else maxizq = maxizq - 1;
        if (maxder == 0) {
            maxder = maxcamino(a->right);
        }
        else maxder = maxder - 1;
        if (maxizq == maxder)fin = true;
        if (maxizq > maxder)hey = true;
        else hey = false;
        return maxizq + maxder + 1;
    }
    int maxcamino(Link a) {
        int s = 0;
        int max = 0;
        cuencamino(a, s, max);
        return max;
    }
    void cuencamino(Link a, int s, int& max) {
        if (a != nullptr) {
            cuencamino(a->left, s + 1, max);
            cuencamino(a->right, s + 1, max);
        }
        else {
            if (max < s)max = s;
        }
